...
4. **`"use server"` sync-export + monorepo tsconfig exclude are tsc-invisible** (#18/#19). Only a full `next build` catches them. → *a pre-deploy `next build` CI gate.*
5. **Silent lab drops are logged, not prevented** (#16). A renamed Zenoti service or expired session produces no card; the safety is a `fly logs` line. → *turn the `console.warn` into a watchdog alert.*
