...
| Position text on a req-form template | Calibrate visually — drag fields in `ReqFormCalibrator.tsx`; positions persist via `req-forms/overrides.ts` (JSON in the templates bucket) and `fillReqForm` merges them over `specs.ts`. | Don't hand-tune coords in `specs.ts` by eyeballing previews. SVG `<text>` is anchored at its alphabetic baseline = pdf-lib's `drawText` anchor, so the overlay is WYSIWYG. pdf.js renders the page; the worker is `public/pdf.worker.min.mjs` (auto-copied by the `copy-pdf-worker` npm prebuild/predev hook — never let it drift from the installed pdfjs-dist version). Overrides are live; bake settled numbers back into `specs.ts` as the code source of truth. |
