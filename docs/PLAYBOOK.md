...
| Which cards need / have a pickup | `awaitingPickup` / `pickupPending` — `src/lib/labs/pickup.ts` | Shared by SchedulePickupButton + TrackingBoard's "Pending pickup" column. Candidates = tracking# + no booking + not scanned into network + no results back. **Don't** filter on `step1_sample_sent` (it auto-ticks on tracking entry) or on bare `tracking_number` (counts all history — the "Schedule pickup (109)" bug). One Book click = ONE FedEx API call, stamps only selected cards. |
