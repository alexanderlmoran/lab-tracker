// Core "recompute lab turnarounds from observed history" logic, extracted from
// the user-facing server action (src/app/labs/learn-actions.ts) so the Vercel
// cron route (/api/cron/learn-turnarounds) can call it without going through
// requireRole (cron has no user; it auths via CRON_SECRET on the route handler).
//
// No "use server" directive here on purpose — this is a plain library function,
// callable from the server action and the cron route alike. Same split pattern
// as src/lib/tracking/refresh-core.ts.
//
// This module owns NO auth and NO cache/revalidation side effects beyond the
// labs_catalog writes. Each caller is responsible for:
//   - gating access (the action does requireRole("developer"); the cron route
//     checks CRON_SECRET)
//   - invalidating the effective-catalog cache + revalidating UI paths so the
//     newly-learned turnarounds show up (the action revalidatePaths; the cron
//     route just invalidates the in-memory cache).
 ...
 *
 * Pure-ish: this reads lab_events / lab_cases and writes labs_catalog. It does
 * NOT auth, revalidate, or touch the effective-catalog cache — callers own
 * those. See module header.
 */
export async function recomputeTurnaroundsCore(): Promise<
