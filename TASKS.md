...
**DONE (ready to deploy) — board column / step RE-SEQUENCE (Alex 2026-06-24):** `1. To Do · 2. Ready to Ship · 3. With Patient · 4. Sample Sent · 5. Pending Upload · 6. Upload Complete · (keep 7+)`. Implemented as **Option (b)**: NO step renumber (Nadia trigger / `/9` counter / result pipeline untouched). Relabel `untouched→To Do`, `complete_results→Upload Complete`; new derived lane order; **one new field `lab_cases.with_patient_at`** (migration `20260624_with_patient_stage.sql`) set by a staff "Given to patient" tap (checklist toggle + drag-to-lane). Commit `17a8bcf` on `deploy/audit-fixes`. xhigh review found 7 bugs — ALL FIXED (isReadyToShip excludes with_patient so pickup/tracking agree; drag calls setWithPatient; req-form button shows in with_patient; step1 clears with_patient_at; checklist row pre-ship-only; double-tap guard; distinct pink color). Deploy: migration + `vercel --prod` (app-only). Was: needed a design pass — done 2026-06-24.
...
**UI DONE 2026-06-05 (commit e4ac609):** a "⊕ Merge dupes" toggle (shown only when dups exist) collapses same-accession dup cards WITHIN a column into one elongated merged card (shared ACC# + every sibling's tracking #); all other patients' cards stay visible. Clicking opens the lead case; actions cascade to the group. Remaining nicety (not built): the toggle currently lives in the board (top-right) rather than next to the page-level "By patient" tab, and merge is per-column (siblings in different columns aren't merged until the cascade converges them).

### Vibrant multi-section download format — SOLVED 2026-06-08 (HAR capture)
**FIXED.** Capture `captures/vibrant/20260608-095759` showed the portal's "Download all reports" hits `/#/printable/AllSummaryReport/<acc>?startPageNumber=1&&localTimeZone=America/New_York&&jwtToken=<jwt>` — renders the WHOLE order (every finished section) in one PDF, NO section codes. `vibrant.ts:downloadPdf()` now uses this route; `run()` drops the per-section logic and `probeByName` no longer restricts to single-section orders (multi-section complete orders auto-post too). Verified live: JAMES FRANGI `2604036550` → 36-page / 11.2 MB real report, `errors:[]`, `isPartial:false`. (For the record, the old per-section CUSTOM route needs codes like `OAC.cbrf-OSU.cbrf-CDZ.cbrf` — `.cbrf` suffix, hyphen-joined — NOT bare comma-joined short_names; AllSummaryReport sidesteps all of that.) **Note:** every Vibrant PDF staged before this fix was an error page (none real); the stale ones were swept (`sweep-vibrant-bad-pdfs.ts`).
...
### Kennedy Krieger — email-only lab automation (Alex's TOP priority, 2026-06-04)
KK results arrive ONLY by email from **geneticslab@kennedykrieger.org** with a **password-protected PDF** (password **"KKI"/"kki"**). Today Alex manually forwards that PDF to **results@bodybio.com**, who turn the raw PDF into a patient-side report. Gmail ingest already exists (`src/lib/gmail/{client,sync}.ts`, `/labs/inbox`, OAuth) and the **Inbox tab is back in the nav** (commit be6ea2f) — but it has NO KK handling, and `extractPdfText` will choke on the encrypted PDF.
- **Phase A — IN PROGRESS. Step 1 DONE + VERIFIED:** manual "Forward to BodyBio" button (re-fetches the PDF from Gmail, sends via Resend with the PDF attached). Alex verified it 2026-06-04 (received the PDF). `KK_FORWARD_TO=results@bodybio.com` is LIVE (forward no longer goes to the test address). The forward does NOT need to decrypt (BodyBio handles the password); decrypt ("kki", lowercase) is only for parsing the patient name to match a case.
  - **REMAINING for full automation (build carefully — outward-facing: auto-emails BodyBio, posts to patient charts, escalates to staff):**
    1. **Schedule the Gmail sync** — currently manual "Sync now"; needs a worker loop/cron (same "must be scheduled" lesson).
    2. **Auto-forward** in the sync when a KK email with a PDF is detected + not yet forwarded → forward + mark `applied_action="kk_forwarded_bodybio"` + timestamp (no migration; reuse applied_action/reviewed_at).
    3. **Catch BodyBio reply** (results@bodybio.com, no subject) → match to patient (name from PDF/subject) → post to PB → Complete Upload. (Keep the human-Approve gate, or auto per Alex.)
    4. **ESCALATION checkpoints (Alex 2026-06-04):** forwarded-but-no-reply → after **~2 days FLAG/notify** → after **~1–2 weeks ESCALATE** to **allison@centnerwellness.com + chrisc@centnerwellness.com + alex@centnerhb.com**. Daily cron finds overdue forwards.
- **Phase B — DEACTIVATED 2026-06-04** (Alex). Do NOT build unless re-opened.

### Gmail ingest revival — DONE (tab back); polling cadence TBD
Inbox tab restored. Confirm Gmail OAuth is connected for labs@centnerhb.com and that `syncGmailInbox` is actually scheduled (same "must be a [processes]/cron loop" lesson — verify it runs, not just exists).

### (3) FedEx schedule-pickup — API BUILT but BLOCKED → pivoting to web scrape
**API path (built, parked):** "Schedule pickup" button (`src/app/labs/SchedulePickupButton.tsx`) → `scheduleFedexPickup` → `src/lib/tracking/fedex-pickup.ts` (Create Pickup API, own OAuth via `FEDEX_PICKUP_API_KEY/SECRET`). Config in Vercel: account **20847088**, Brickell address, contact Centner Wellness / 305-602-5260, window 2:30–4:30. **Proven locally** (`worker/scripts/pickup-local.ts`): OAuth succeeds + request body is accepted, but Create Pickup returns `400 SHIPMENT.USER.UNAUTHORIZED — Requested user is not authorized`. So the code is correct; the FedEx **account/project isn't authorized for pickup** (link account 20847088 to the API project / get the Pickup product *approved* not just added / move project to Production). Gotcha: FedEx always gzips error bodies — decompress to read them. Also flaky: today's Vercel env wasn't reaching builds (the prod button is unverified; fix only matters if we keep the API path).

**DECISION 2026-06-04: scrap the API, SCRAPE fedex.com instead.** Browser-automate the web "Schedule a pickup" flow with the web login **CW2333** (creds → env/Fly secrets, NEVER committed; rotate the password that was shared in chat). ⚠️ fedex.com runs **Akamai Bot Manager** (saw `_abck`/`bm_sz` cookies in the API responses) — headless Playwright will likely be challenged, so capture-with-real-login first (codegen + HAR), then a deterministic scraper. Same pattern as the lab portals.
