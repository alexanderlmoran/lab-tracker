...
- **Phase A (automate the current flow):** Gmail sync detects sender `geneticslab@kennedykrieger.org` → **decrypt the PDF with "KKI"** (qpdf / node-qpdf2 — pdf-lib can't decrypt) → **forward the decrypted PDF to results@bodybio.com** (Resend is already wired) → attach to the patient's KennedyKrieger case + log. Replaces the manual forward. NEED FROM ALEX: a **sample KK email + PDF** to build/test the decrypt + forward, and confirm labs@centnerhb.com Gmail is OAuth-connected.
- **Phase B (replace BodyBio — ambitious):** generate the finished patient report OURSELVES from the raw KK PDF via AI, using sample finished reports as templates. NEED FROM ALEX: **several (raw KK PDF → finished BodyBio report) pairs** so the model learns the transformation. Big feature; do after Phase A proves the ingest.
